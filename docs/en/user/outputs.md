...
| `*_auto_segmentation_feature_*.nii.gz` | auto segmentation succeeds | saved nnUNet feature-channel NIfTI inputs used by the run in the same `HF/AP/RL` geometry convention used by the nnUNet training/export scripts |
