...
normal Qt widget. Left-drag rotates, middle-drag pans, right-drag or the mouse
wheel zooms, and existing 3D picking and edit controls remain available. A
local display such as `DISPLAY=:1` continues to use the native embedded VTK
...
The left browser is group-aware. When the loaded segmentation produces multiple vessel groups, the browser shows one top-level section per group, then type sections such as `Paths`, `Planes`, and `Pathlines`, so you can toggle a whole group or one object class at once. Use Ctrl/Shift selection with a plane context menu to launch pathlines for that subset; the main `Pathlines` action runs all planes.
